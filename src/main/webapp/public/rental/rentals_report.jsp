...
<div class="row g-4 align-items-start">
    <div class="col-lg-4">
        <div class="card shadow-soft analytics-card">
            <div class="card-body">
                <h2 class="h5 fw-semibold mb-3"><fmt:message key="rental.public.report.sidebar.title" /></h2>
                <p class="text-muted"><fmt:message key="rental.public.report.sidebar.description" /></p>
                <form method="get" action="${ctx}/public/rentals" class="analytics-form">
                    <div class="mb-3">
                        <label for="status" class="form-label"><fmt:message key="rental.public.report.filter.status.label" /></label>
                        <select class="form-select" id="status" name="${paramStatus}">
                            <option value=""><fmt:message key="rental.public.report.filter.status.all" /></option>
                    ...
                    <div class="row g-2 mb-3">
                        <div class="col">
                            <label for="startFrom" class="form-label"><fmt:message key="rental.public.report.filter.startFrom" /></label>
                            <input type="date" class="form-control" id="startFrom"
                                   name="${paramStartFrom}"
                                   value="${filters[paramStartFrom]}">
                        </div>
                        <div class="col">
                            <label for="startTo" class="form-label"><fmt:message key="rental.public.report.filter.startTo" /></label>
                    ...
                    <div class="row g-2 mb-4">
                        <div class="col">
                            <label for="minCost" class="form-label"><fmt:message key="rental.public.report.filter.minCost" /></label>
                            <input type="number" step="0.01" min="0" class="form-control" id="minCost"
                                   name="${paramMinCost}"
                                   value="${filters[paramMinCost]}">
                        </div>
                        <div class="col">
                            <label for="maxCost" class="form-label"><fmt:message key="rental.public.report.filter.maxCost" /></label>
                    ...
                    <div class="d-grid gap-2">
                        <button type="submit" class="btn btn-brand"><fmt:message key="rental.public.report.actions.apply" /></button>
                        <a class="btn btn-outline-brand" href="${ctx}/public/rentals"><fmt:message key="rental.public.report.actions.clear" /></a>
                    </div>
                </form>
    ...
    <div class="col-lg-8">
        <div class="row g-3 mb-4">
            <div class="col-md-4">
                <div class="analytics-stat">
                    <span class="analytics-label"><fmt:message key="rental.public.report.stat.filtered" /></span>
                    <span class="analytics-value">${summary.totalRentals}</span>
                </div>
            </div>
            <div class="col-md-4">
                <div class="analytics-stat">
                    <span class="analytics-label"><fmt:message key="rental.public.report.stat.revenue" /></span>
            ...
            <div class="col-md-4">
                <div class="analytics-stat">
                    <span class="analytics-label"><fmt:message key="rental.public.report.stat.duration" /></span>
                    <span class="analytics-value">
                        <fmt:message key="rental.public.report.stat.duration.value">
                            <fmt:param>
        ...
        <div class="card card-common mb-4">
            <div class="card-header"><fmt:message key="rental.public.report.section.statusDistribution" /></div>
            <div class="card-body">
                <div class="row g-3">
                    <c:forEach var="status" items="${statusOptions}">
        ...
        <div class="card card-common mb-4">
            <div class="card-header"><fmt:message key="rental.public.report.section.latest" /></div>
            <div class="card-body">
                <c:choose>
                    <c:when test="${empty latestRentals}">
                        <p class="text-muted mb-0"><fmt:message key="rental.public.report.section.latest.empty" /></p>
                    </c:when>
                    <c:otherwise>
        ...
        <div class="card card-common">
            <div class="card-header"><fmt:message key="rental.public.report.section.details" /></div>
            <div class="card-body p-0">
                <div class="table-responsive">
                    <table class="table table-hover mb-0 align-middle">
                        <thead class="table-light">
                            <tr>
                                <th><fmt:message key="rental.public.report.table.header.id" /></th>
                                <th><fmt:message key="rental.public.report.table.header.vehicle" /></th>
                                <th><fmt:message key="rental.public.report.table.header.status" /></th>
                                <th><fmt:message key="rental.public.report.table.header.start" /></th>
                                <th><fmt:message key="rental.public.report.table.header.end" /></th>
                                <th class="text-end"><fmt:message key="rental.public.report.table.header.amount" /></th>
                            </tr>
                        </thead>
                        <tbody>
                            <c:if test="${empty rentals}">
                                <tr>
                                    <td colspan="6" class="text-center text-muted py-4"><fmt:message key="rental.public.report.table.empty" /></td>
                                </tr>
                            </c:if>
