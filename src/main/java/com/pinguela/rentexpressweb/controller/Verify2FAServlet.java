...
public class Verify2FAServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		request.setAttribute(AppConstants.ATTR_PAGE_TITLE, MessageResolver.getMessage(request, "page.verify2fa.title"));
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (sanitizedCode.isEmpty()) {
			errors.put(ERROR_KEY_CODE, MessageResolver.getMessage(request, "validation.verify2fa.code.required"));
		} else if (!sanitizedCode.matches("\\d{" + SecurityConstants.TWO_FA_CODE_LENGTH + "}")) {
			errors.put(ERROR_KEY_CODE, MessageResolver.getMessage(request, "validation.verify2fa.code.length",
					Integer.valueOf(SecurityConstants.TWO_FA_CODE_LENGTH)));
		}

		if (errors.isEmpty() && TwoFactorManager.isExpired(request)) {
			errors.put(ERROR_KEY_GLOBAL, MessageResolver.getMessage(request, "validation.verify2fa.code.expired"));
		}

		if (errors.isEmpty() && !TwoFactorManager.matchesCode(request, sanitizedCode)) {
			errors.put(ERROR_KEY_GLOBAL, MessageResolver.getMessage(request, "validation.verify2fa.code.invalid"));
		}
		...
		if (!errors.isEmpty()) {
			...
			request.setAttribute(ATTR_SECONDS_REMAINING,
					Long.valueOf(TwoFactorManager.secondsUntilExpiration(request)));
			request.getRequestDispatcher(VIEW_VERIFY_2FA).forward(request, response);
			return;
		}
